    ...
    def delete_models(self, queryset):
        ...
        if len(queryset) > 0:
            ...
            for line in queryset:
                receiveLineQuerySet = ReceivingLine.objects.filter(orderLine__id = line.id)
                if len(receiveLineQuerySet) > 0:
                    line.documentLineItem.posted_quantity -= (receiveLineQuerySet[0].receiving_quantity or 0)
                    line.documentLineItem.save(update_fields=['posted_quantity'])
