...
class Payment(models.Model):
    ...
    payment_user = models.ForeignKey(UserInfo, related_name='paymentGroup', verbose_name=u'付款经办人', limit_choices_to = Q( groups__name = u'采购部门'))
    purchase_user = models.ForeignKey(UserInfo, related_name='purchaseGroup', verbose_name=u'采购经手人', limit_choices_to = Q( groups__name = u'采购部门'), blank=True, null=True)
    ...
    create_user = models.ForeignKey(UserInfo,verbose_name=u'申请人')
    
    def save(self, *args, **kwargs):
        if self.vendor:
            self.account = self.vendor.account
            self.bank = self.vendor.bank
