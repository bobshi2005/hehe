...
class PaymentAdmin(object):
    ...
    def save_models(self):
        ...
        if not bool(self.new_obj.payment_id):
            ...
            self.new_obj.owed_amount = getOwedAmount(year, month,  self.new_obj.vendor)
            self.new_obj.owed_amount_after_payment = self.new_obj.owed_amount - self.new_obj.payment_amount
            self.new_obj.create_user = self.user
        else:
            #更新
    ...
    def doAction(self, instance):
        #申请人自己才能点提交
        if (not instance.is_applied or paymentAuditStatus(instance, ITEM_REJECTED)) and (instance.create_user == self.user):
    ...
    def readonly_fields(self):
        result = ('owed_amount', 'owed_amount_after_payment', 'applied_amount', 'bank', 'account')
        ...
        if hasattr(self, 'org_obj') and paymentAuditStatus(self.org_obj, ITEM_APPROVED):
            result = ('company', 'vendor', 'content', 'owed_amount', 'payment_amount', 'owed_amount_after_payment', 
                      'paymentProperty', 'purchase_amount', 'applied_amount', 'purchase_user', 'paymentType', 'payment_user', 'comments', 'bank', 'account')
        #在审批中 并且是采购部门
        ...
        #在审批中 并且不是采购部门
            ...
            result = ('company', 'vendor', 'content', 'owed_amount', 'payment_amount', 'owed_amount_after_payment', 
                      'paymentProperty', 'purchase_amount', 'purchase_user', 'paymentType', 'payment_user', 'comments', 'bank', 'account')
